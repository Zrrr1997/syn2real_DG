...
def generate_heatmaps_and_limbs(H=640, W=368, paths=None, root_dir=None, result_dir=None):
	...
	for path in tqdm(paths): 
		...
		if not os.path.exists(os.path.join(result_dir, 'optical_flow', os.path.basename(path[:-4]))):
			os.mkdir(os.path.join(result_dir, 'optical_flow', os.path.basename(path[:-4])))

		writer_optical_flow = cv2.VideoWriter(os.path.join(result_dir, 'optical_flow', os.path.basename(path[:-4]), 'optical_flow.avi'), cv2.VideoWriter_fourcc(*"MJPG"), 30, (W, H))
		print("Writing in:", os.path.join(result_dir, 'optical_flow', os.path.basename(path[:-4]), 'optical_flow.avi'))
...
if __name__ == '__main__':
	...
	paths = [os.path.join(args.root_dir, el) for el in sorted(os.listdir(args.root_dir))] # or only paths... (see last edit note)
