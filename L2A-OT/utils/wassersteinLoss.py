...
def sink(M, reg = 1, numItermax=1000, stopThr=1e-9, cuda = True, device=None):
    ...
    if cuda:
        u = Variable(torch.ones(Nini) / Nini).to(device)
        v = Variable(torch.ones(Nfin) / Nfin).to(device)
    else:
...
def sink_stabilized(M, reg, numItermax=1000, tau=1e2, stopThr=1e-9, warmstart=None, print_period=20, cuda=True, device=None):
