...
def get_args():
    ...
    train_arg_parser.add_argument("--num_classes", type=int, default=10,
                                  help="")
    train_arg_parser.add_argument("--num_frames_video", type=int, default=10,
                                  help="")
    ...
    train_arg_parser.add_argument('--save_path', default='debug', type=str, help="Video folder for the output tSNE visualization.")
    train_arg_parser.add_argument('--video_paths', default=None, nargs='+', type=str, help="Video path of video to be processed.")
...
class L2A_OT_Trainer(object):
    ...
    def useG(self, video_paths, save_path, num_frames_video):
        ...
        for video_path in tqdm(video_paths):
                ...
                index = 0
                for i in range(4):
                    if modalities[i] + '.avi' in video_path or modalities[i] + '.mp4' in video_path:
                        index = i
                ...
                while t < num_frames_video and ret:
                    ret, frame = cap.read()
                    if not ret or frame is None:
                       print(modalities[index])
                       continue
                    if np.sum(frame) == 0:
                       empty_frames += 1
                       continue
                    ...
                    embedding = self.G.embed(x_real, label_trg).flatten().cpu().data.numpy()
                    embeddings.append(embedding)
                    labels.append(index)


                    x_fake = self.G(x_real, label_trg)
                    embedding_target = self.G.embed(x_fake, label_org).flatten().cpu().data.numpy()
                    embeddings_target.append(embedding_target)
                    labels_target.append(index + 4)

                
                #np.save(os.path.join('embeddings', 'source_modality_' + modalities[self.modality_indices[0]] + '.npy'), embeddings)
                #np.save(os.path.join('embeddings', 'target_modality_' + modalities[self.modality_indices[0]] + '.npy'), embeddings_target)
        if not os.path.exists(os.path.join('embeddings', save_path)):
            os.mkdir(os.path.join('embeddings', save_path))
        print("Skipped", empty_frames, "empty frames...")
        print(np.array(embeddings).shape, np.array(embeddings_target).shape)
        embeddings = np.concatenate((np.array(embeddings), np.array(embeddings_target)), axis=0)
        print("Concatenated embeddings...")
        labels = np.concatenate((np.array(labels), np.array(labels_target)), axis=0)
        print("Concatenated labels...")
        tSNE(os.path.join('embeddings', save_path, 'tsne.png'), embeddings, labels)
...
def main():
    ...
    if '.txt' in args.video_paths or True:
       f = open(args.video_paths[0], 'r')
       video_paths = np.array([el.rstrip() for el in f.readlines() if 'avi' in el or 'mp4' in el])
       video_paths = video_paths[1::8 * 15] #8*15 for adl
       print("#Heatmaps", len([el for el in video_paths if 'heatmaps.avi' in el]))
       print("#Limbs", len([el for el in video_paths if 'limbs.avi' in el]))
       print("#Optical Flow", len([el for el in video_paths if 'optical_flow.avi' in el]))
       print("#RGB", len([el for el in video_paths if 'rgb.mp4' in el or 'rgb.avi' in el]))


       print("---------------------------------------------------")
       print('Using ', len(video_paths), 'video samples')
       print("---------------------------------------------------")
       
    else:
       video_paths = args.video_paths
    trainer.useG(video_paths, args.save_path, args.num_frames_video)
